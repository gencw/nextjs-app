...
function MenubarItem({
  ...
  return (
    <DropdownMenuItem
      ...
      className={cn(
        "group/menubar-item gap-1.5 rounded-md px-1.5 py-1 text-sm outline-hidden select-none focus:bg-accent focus:text-accent-foreground not-data-[variant=destructive]:focus:**text-accent-foreground data-inset:pl-7 data-[variant=destructive]:text-destructive data-[variant=destructive]:focus:bg-destructive/10 data-[variant=destructive]:focus:text-destructive dark:data-[variant=destructive]:focus:bg-destructive/20 data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 data-[variant=destructive]:*:[svg]:text-destructive",
        className
      )}
      {...props}
    />
...
function MenubarCheckboxItem({
  ...
  inset,
  ...props
}: MenuPrimitive.Checkbox.Item.Props & {
  inset?: boolean
}) {
  return (
...
function MenubarRadioItem({
  ...
  inset,
  ...props
}: MenuPrimitive.Radio.Item.Props & {
  inset?: boolean
}) {
  return (
    <MenuPrimitive.Radio.Item
    ...
    >
      <span className="pointer-events-none absolute right-2 flex items-center justify-center [&_svg:not([class*='size-'])]:size-4">
        <MenuPrimitive.Radio.ItemIndicator>
          <CheckIcon
          />
        </MenuPrimitive.Radio.ItemIndicator>
      </span>
